...
class LevelButtonFragment: Fragment(), BuyDialogCallback {
    ...
    private fun update() {
        ...
        if (visible){
            ...
            level = gson.fromJson(settingPrefs.getString(String.format("level%d", buttonIndex+1), gson.toJson(level)), Level::class.java)
        } else {
        ...
        if (level.locked){
            ...
            if (level.tried){
                if (level.time < 0){
                    button.background.setTintList(ContextCompat.getColorStateList(requireContext(),
                        R.color.red
                    ))
                } else {
                    button.background.setTintList(ContextCompat.getColorStateList(requireContext(),
